...
async function sendEditorForm(data, imageEditorDiv) {
    ...
    try
    {
        ...
        if (response.ok)
        {
            addImageToFeed(resData.image_metadata, imageEditorDiv);
            return ;
        }
    }
    catch (error)
...
async function deleteImageFromFeed(image, element) {
    ...
    try
    {
        ...
        if (response.ok)
            document.getElementById("feed").removeChild(element);
    }
    catch (error)
    {
...
function addImageToFeed(image, imageEditorDiv) {
    ...
    deleteBtn.innerHTML = `
    <svg fill="#ffffff" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
...
`;
    ...
    deleteBtn.addEventListener("click", async () => {
        ...
        await deleteImageFromFeed(image, newDiv)
...
async function loadUserFeed(imageEditorDiv) {
    const userImages = await getUserFeed();
    if (!userImages)
        return;
    ...
    for (let i = 0; i < userImages.length; i++)
        addImageToFeed(userImages[i], imageEditorDiv);
}
...
async function loadOverlays(imageEditorDiv) {
    ...
    // Charge les overlays sur l'UI de l'editor.
    const overlaysDiv = imageEditorDiv.querySelector("fieldset[id=overlays]");
    for (let i = 0; i < overlays.length; i++)
    {
        const overlayMiniatureDiv = document.createElement("div");
        const overlayMiniatureInput = document.createElement("input");
        const overlayMiniatureLabel = document.createElement("label");
        const overlayMiniatureImg = document.createElement("img");

        // Input setup
        overlayMiniatureInput.id = overlays[i].id;
        ...
        overlayMiniatureImg.src = overlays[i].path;

        // Adding to view
        overlayMiniatureLabel.appendChild(overlayMiniatureImg);
        overlayMiniatureDiv.appendChild(overlayMiniatureInput);
        overlayMiniatureDiv.appendChild(overlayMiniatureLabel);
        overlaysDiv.appendChild(overlayMiniatureDiv);
    }
}
